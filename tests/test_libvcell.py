...
from libvcell import sbml_to_finite_volume_input, vcml_to_finite_volume_input, sbml_to_vcml, vcml_to_sbml, vcml_to_vcml
...
def test_sbml_to_vcml(temp_output_dir: Path, sbml_file_path: Path) -> None:
    ...
    success, msg = sbml_to_vcml(
        sbml_content=sbml_content, vcml_file_path=vcml_file_path, validate=False
    )
    assert len(list(temp_output_dir.iterdir())) == 1
    assert success is True
    assert msg == "Success"
...
def test_vcml_to_vcml(temp_output_dir: Path, vcml_file_path: Path) -> None:
    vcml_content = vcml_file_path.read_text()
    vcml_file_path = temp_output_dir / "test.vcml"
    success, msg = vcml_to_vcml(
        vcml_content=vcml_content, vcml_file_path=vcml_file_path
    )
    assert len(list(temp_output_dir.iterdir())) == 1
    assert success is True
    assert msg == "Success"
